...
def readelf(elf):
	...
	print("Number of program headers:         %d" % e_phnum)
	...
	# print "=== 0x%x ===" % (e_shoff + e_shentsize * e_shstrndx)
	if ei_class == 1:
		sh_name, sh_type, sh_flags, sh_addr, sh_offset, sh_size, sh_link, sh_info, sh_addralign, sh_entsize  = struct.unpack('IIIIIIIIII', elf.read(40))
	else:
	...
	# print "==="  + "sh_name=0x%x" %sh_name + " sh_type=0x%x"%sh_type + " sh_flags=0x%x" %sh_flags + " sh_addr=0x%x" %sh_addr + " sh_offset=0x%x" %sh_offset + " sh_size=0x%x" %sh_size
	elf.seek(sh_offset)
	...
	for i in range(0, e_phnum):
		...
		if ei_class == 1:
			p_type, p_offset, p_vaddr, p_paddr, p_filesz, p_memsz, p_flags, p_align = struct.unpack('IIIIIIII', elf.read(32))
		else:
	...
	if e_shdynsym >= 0 and e_shdynstr >= 0:
		...
		for i in range(0, sh_size / 24):
			if ei_class == 1:
				st_name, st_value, st_size, st_info, st_other, st_shndx = struct.unpack('IIIBBH', dynsym_section[i*16:(i+1)*16])
			else:
				st_name, st_value, st_size, st_info, st_other, st_shndx = struct.unpack('IBBHQQ', dynsym_section[i*24:(i+1)*24])

			if st_name in dynsymbol_table:
				print("%04d%10d%10d%10s%10s%10s%10d%30s" %(i, st_value, st_size, STT_TYPE[ELF_ST_TYPE(st_info)], STB_BIND[ELF_ST_BIND(st_info)],
															  STV_VISIBILITY[ELF_ST_VISIBILITY(st_other)], st_shndx, dynsymbol_table[st_name]))
			else:
	...
	if e_shdynamic >= 0:
		...
		print('\nDynamic section:')
		print('%10s %20s %20s' %("Tag", "Type", "Name/Value"))
		if ei_class == 1:
			for i in range(0, sh_size/8):
				elf.seek(sh_offset + i * 8)
				d_tag, d_un = struct.unpack('II', elf.read(8))
				if d_tag in TAG:
					if d_tag == 1 or d_tag == 15:
						print('0x%08x %20s %20s' %(d_tag, TAG[d_tag], dynsymbol_table[d_un]))
					else:
						print('0x%08x %20s %18x' %(d_tag, TAG[d_tag], d_un))
				else:
					if d_tag == 1 or d_tag == 15:
						print('0x%018x %20s %20s' %(d_tag, d_tag, dynsymbol_table[d_un]))
					else:
						print('0x%018x %20s %18x' %(d_tag, d_tag, d_un))
		else:
